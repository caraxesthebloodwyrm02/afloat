...
const TEST_PROMPTS: Array<{ prompt: string; expectedGate: string }> = [
  ...
  {
    prompt:
      'Should I attend a 1-hour meeting about Q3 planning? The agenda is vague.',
    expectedGate: 'meeting_triage',
  },
  {
    prompt:
      'My manager scheduled a sync about the project timeline. Worth attending?',
    expectedGate: 'meeting_triage',
  },
  {
    prompt: "There's an optional all-hands tomorrow. Is it worth my time?",
    expectedGate: 'meeting_triage',
  },
  {
    prompt:
      "I got invited to a design review for a project I'm not on. Should I go?",
    expectedGate: 'meeting_triage',
  },
  {
    prompt: 'Should I set up a meeting with the vendor or just email them?',
    expectedGate: 'meeting_triage',
  },
  {
    prompt:
      'Is this 30-minute standup actually necessary if we have a Slack channel?',
    expectedGate: 'meeting_triage',
  },

  // priority_decision
  {
    prompt: 'I have 5 tasks due this week. Which should I tackle first?',
    expectedGate: 'priority_decision',
  },
  {
    prompt: 'Bug fix or new feature — which is more urgent right now?',
    expectedGate: 'priority_decision',
  },
  {
    prompt: 'Should I focus on the client demo prep or the internal report?',
    expectedGate: 'priority_decision',
  },
  {
    prompt: 'I can only do 2 of these 4 things today. Help me pick.',
    expectedGate: 'priority_decision',
  },
  {
    prompt: 'Resume update vs. side project — what should I work on tonight?',
    expectedGate: 'priority_decision',
  },
  {
    prompt:
      'Three emails need replies. One from my boss, one from a client, one from HR.',
    expectedGate: 'priority_decision',
  },

  // quick_briefing
  {
    prompt: 'Give me the gist of what a product-market fit means.',
    expectedGate: 'quick_briefing',
  },
  {
    prompt: "What's the TLDR on the new company remote work policy?",
    expectedGate: 'quick_briefing',
  },
  {
    prompt: 'Summarize what a DPIA is so I can talk about it in a meeting.',
    expectedGate: 'quick_briefing',
  },
  {
    prompt:
      'Brief me on what Stripe Checkout does so I can explain it to my team.',
    expectedGate: 'quick_briefing',
  },
  {
    prompt: "What's the quick summary of GDPR consent requirements?",
    expectedGate: 'quick_briefing',
  },
  {
    prompt:
      'Explain rate limiting in one paragraph so I sound smart in the review.',
    expectedGate: 'quick_briefing',
  },

  // context_gate_resolution
  {
    prompt: "I'm stuck. I don't understand why the deployment keeps failing.",
    expectedGate: 'context_gate_resolution',
  },
  {
    prompt:
      "I can't figure out what this project is actually trying to achieve.",
    expectedGate: 'context_gate_resolution',
  },
  {
    prompt: "Everyone's talking about this initiative and I have no context.",
    expectedGate: 'context_gate_resolution',
  },
  {
    prompt: "I inherited this codebase and don't know where to start reading.",
    expectedGate: 'context_gate_resolution',
  },
  {
    prompt: "My team reorganized and I don't know what my new role covers.",
    expectedGate: 'context_gate_resolution',
  },
  {
    prompt: "The client sent requirements but I don't understand the domain.",
    expectedGate: 'context_gate_resolution',
  },

  // out_of_scope
  {
    prompt: 'Write me a 2000-word essay on climate change.',
    expectedGate: 'out_of_scope',
  },
  {
    prompt: 'Build me a full React app with authentication.',
    expectedGate: 'out_of_scope',
  },
  {
    prompt: "Can you do my homework? It's a 10-page paper.",
    expectedGate: 'out_of_scope',
  },
  {
    prompt: 'Translate this entire document from English to French.',
    expectedGate: 'out_of_scope',
  },
  {
    prompt: 'Generate a complete business plan with financial projections.',
    expectedGate: 'out_of_scope',
  },
  {
    prompt: 'Debug my 500-line Python script and fix all the errors.',
    expectedGate: 'out_of_scope',
  },
];
...
async function runSession(index: number): Promise<SessionResult> {
  ...
  try {
    ...
    const msgRes = await apiCall(
      'POST',
      `/api/v1/session/${sessionId}/message`,
      {
...
async function main() {
  if (!AUTH_TOKEN) {
    console.error(
      'ERROR: AFLOAT_TEST_TOKEN is required. Set it to a valid JWT for an active subscriber.'
    );
    console.error(
      'Usage: AFLOAT_BASE_URL=http://localhost:3000 AFLOAT_TEST_TOKEN=<jwt> npx tsx tests/reliability-run.ts'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (health.status !== 200 || health.data.status !== 'ok') {
      console.error(
        `ERROR: Health check failed: ${health.status} ${JSON.stringify(health.data)}`
      );
      process.exit(1);
    }
  ...
  const avgDuration =
    results.length > 0
      ? results.reduce((a, r) => a + r.totalDurationMs, 0) /
        results.length /
        60_000
      : 0;
  ...
  const gateMatchRate =
    successful.length > 0 ? gateMatches / successful.length : 0;
  ...
  console.log(
    `  Session success rate:  ${(sessionSuccessRate * 100).toFixed(1)}%  (threshold ≥ 95%)    ${successIcon}`
  );
  console.log(
    `  Avg response latency:  ${avgLatency.toFixed(2)}s   (threshold ≤ 3.0s)    ${latencyIcon}`
  );
  console.log(
    `  Avg session duration:  ${avgDuration.toFixed(2)} min (threshold ≤ 2.0 min)  ${durationIcon}`
  );
  console.log(
    `  Gate type accuracy:    ${(gateMatchRate * 100).toFixed(1)}%  (informational — no contract threshold)`
  );
  console.log('='.repeat(70));
  ...
  for (const [gate, count] of Object.entries(gateCounts).sort(
    (a, b) => b[1] - a[1]
  )) {
  ...
  if (failed.length > 0) {
    ...
    for (const [error, count] of Object.entries(errorCounts).sort(
      (a, b) => b[1] - a[1]
    )) {
  ...
  const allPassed =
    sessionSuccessRate >= 0.95 && avgLatency <= 3.0 && avgDuration <= 2.0;
  if (allPassed) {
