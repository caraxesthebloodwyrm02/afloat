...
vi.mock("@/lib/redis", () => ({
  getRedis: () => ({
    ...
    scan: vi.fn(async (_cursor: number, opts?: { match?: string; count?: number }) => {
      ...
      const keys = [...new Set([...mockKv.keys(), ...mockLists.keys()])].filter((k) =>
        k.startsWith(prefix),
      );
      return [0, keys];
    }),
...
function seedSessionList(date: string, entries: Array<{ user_id: string; session_id: string }>): void {
...
describe("data-layer branch coverage", () => {
  ...
  it("permanently deletes user data through the redis eval branch", async () => {
    const { createUser, setStripeCustomerMapping, permanentlyDeleteUserData } = await import(
      "@/lib/data-layer"
    );
    const user = makeUser("eval-user");
    await createUser(user);
    await setStripeCustomerMapping(user.stripe_customer_id, user.user_id);
    mockKv.set(`routing_profile:${user.user_id}`, JSON.stringify({ user_id: user.user_id }));
    ...
    expect(mockDeleteStripeCustomer).toHaveBeenCalledWith(user.stripe_customer_id);
