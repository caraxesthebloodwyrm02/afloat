...
import {
  createDPR,
  getChainRef,
  serializeDPRForHashing,
} from '@/lib/provenance/record';
...
describe('Hash chain integrity', () => {
  ...
  it('detects tampered chain hash', () => {
    ...
    const serialized = serializeDPRForHashing(
      rest as Parameters<typeof serializeDPRForHashing>[0]
    );
  ...
  it('hashes input content instead of storing raw text', () => {
    const dpr = createDPR(
      makeInput({
        ...
        output_content:
          'Yes, attend. The agenda covers items you need to decide on.',
      }),
      null
    );
