...
describe("enforceSessionLimits", () => {
  it("allows a valid first message", () => {
    ...
    const result = enforceSessionLimits(session, "Should I attend this meeting?");
  ...
  it("rejects when max LLM calls reached", () => {
    const session = makeSession({ llm_call_count: 2 });
  ...
  it("rejects when session has timed out", () => {
    const pastTime = new Date(Date.now() - 130_000).toISOString();
