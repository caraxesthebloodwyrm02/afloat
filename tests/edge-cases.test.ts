...
describe('special character input', () => {
  ...
  it('accepts message with HTML-like content', () => {
    const result = enforceSessionLimits(
      makeSession(),
      "<script>alert('xss')</script> Should I do this?"
    );
    expect(result.allowed).toBe(true);
  });
  ...
  it('accepts message with SQL-like content', () => {
    const result = enforceSessionLimits(
      makeSession(),
      'SELECT * FROM users; DROP TABLE -- should I refactor this query?'
    );
    expect(result.allowed).toBe(true);
  });
...
describe('turn limit boundary conditions', () => {
  ...
  it('rejects beyond turn limit (llm_call_count > MAX_LLM_CALLS)', () => {
    const result = enforceSessionLimits(
      makeSession({ llm_call_count: MAX_LLM_CALLS + 1 }),
      'way too many'
    );
    expect(result.allowed).toBe(false);
    expect(result.errorCode).toBe('session_complete');
...
describe('contract-aligned constants', () => {
  it('MAX_LLM_CALLS matches contract (4)', () => {
    expect(MAX_LLM_CALLS).toBe(4);
  });

  it('MAX_DURATION_MS matches contract (300000ms = 5 minutes)', () => {
    expect(MAX_DURATION_MS).toBe(300_000);
  });
});
